...
class NewsScraper:
  ...
  def scrape_news(tickers, companies):
    ...
    for i in range(len(companies)):
      df = NewsScraper.get_company_news(googlenews, df, one_months_ago, today, companies[i], tickers[i])

      df = NewsScraper.get_company_news(googlenews, df, two_months_ago, one_months_ago_minus_one_day, companies[i], tickers[i])
      
      df = NewsScraper.get_company_news(googlenews, df, three_months_ago, two_months_ago_minus_one_day, companies[i], tickers[i])
      
    df.to_csv("news_data.csv", index=False)
    return df

  def get_company_news(googlenews, df, start, end, company, ticker):
    googlenews.clear()
    ...
    results.datetime.fillna(start, inplace=True) #replace NA with the starting date
